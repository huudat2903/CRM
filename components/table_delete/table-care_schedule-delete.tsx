...
const Table_Care_Schedule = (props: any) => {
  const { ColumCareSchedule, dataPotential, name } = props;
  ...
  const columns = ColumCareSchedule;
  ...
  return (
    <div className={`${styles.main__content} ${styles.flex_column}`}>
      <div className={styles.main__control_search_delete}>
        <Input
          placeholder={`Tìm kiếm theo ${name.toLowerCase()}`}
          suffix={suffix}
          bordered={false}
          style={{
            width: "100%",
...
export default Table_Care_Schedule;
